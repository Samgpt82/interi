...
export default function ResultScreen() {
  ...
  const refinementPending = mutation.isPending || subscription.isPending;
  ...
  const updateRefinement = (value: string) => { if (mutation.isError) mutation.reset(); if (subscription.isError) subscription.reset(); setNotice(null); setRefinement(value); };
  const runRefinement = (instruction: string) => {
    ...
    subscription.mutate(undefined, {
      onSuccess: ({ accessGranted }) => {
        if (accessGranted) mutation.mutate({ sourceImageDataUrl: baseImage, style, roomType, shoppingCountry: result.shoppingCountry, refinement: instruction });
      },
    });
  };
    ...
    <Screen testID="result-screen">
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <ScrollView ref={scrollRef} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 50 }}>
          ...
          {notice ? <Text testID="result-notice" className="mt-3 text-sm" style={{ color: COLORS.oliveDark }}>{notice}</Text> : null}{mutation.isError ? <Text testID="refinement-error" className="mt-3 text-sm" style={{ color: COLORS.coral }}>{mutation.error instanceof Error ? mutation.error.message : 'That refinement did not complete.'}</Text> : null}{subscription.isError ? <Text testID="refinement-subscription-error" className="mt-3 text-sm" style={{ color: COLORS.coral }}>{subscription.error instanceof Error ? subscription.error.message : 'Unable to open subscription options.'}</Text> : null}
