...
export default function SavedScreen() {
  ...
  const filterTile = (id: FolderFilter, label: string, count: number, onActions?: () => void) => {
    const selected = filter === id;

    return (
      <View
        key={id}
        className="min-h-12 flex-row overflow-hidden rounded-[18px] border"
        style={{ width: '48.5%', borderColor: selected ? COLORS.espresso : COLORS.line, backgroundColor: selected ? COLORS.espresso : COLORS.paper }}>
        <Pressable testID={`folder-filter-${id}`} onPress={() => setFilter(id)} className="min-h-12 flex-1 flex-row items-center px-3.5 active:opacity-75">
          <Text numberOfLines={1} className="mr-2 flex-1 text-sm font-semibold" style={{ color: selected ? COLORS.white : COLORS.espresso }}>{label}</Text>
          <Text className="text-xs" style={{ color: selected ? COLORS.sand : COLORS.olive }}>{count}</Text>
        </Pressable>
        {onActions ? <Pressable testID={`folder-actions-${id}`} accessibilityLabel={`Folder options for ${label}`} onPress={onActions} className="min-h-12 w-10 items-center justify-center active:opacity-60"><MoreHorizontal size={17} color={selected ? COLORS.sand : COLORS.olive} /></Pressable> : null}
      </View>
    );
  };
